...
with st.sidebar:
    ...
    with st.expander("🚃 Train Geometry", expanded=True):
        ...
        if config_mode == "Research locomotive model":
            ...
            # 7-mass reference lok (works well / validated)
            ...
            if n_masses == 7:
                # Use validated research model
                masses = default_masses
                x_init = default_x
                y_init = default_y
            else:
                # Generic research train: evenly spaced masses on given length
            ...
            if preset == "ICE3-like EMU":
                default_wagons = 7   # approx. 8-car unit
                default_m_lok = 70.0   # t
                default_m_wag = 48.0   # t
                default_L_lok = 25.0   # m
                default_L_wag = 25.0   # m
            elif preset == "TGV-like set":
                default_wagons = 7
                default_m_lok = 68.0   # t
                default_m_wag = 31.0   # t
                default_L_lok = 22.0   # m
                default_L_wag = 20.0   # m
            elif preset == "TRAXX loco with freight wagons":
                default_wagons = 6
                default_m_lok = 84.0   # t
                default_m_wag = 80.0   # t (loaded wagon)
                default_L_lok = 19.0   # m
                default_L_wag = 15.0   # m
            else:
                # Generic
                default_wagons = 7
                default_m_lok = 80.0   # t
                default_m_wag = 50.0   # t
                default_L_lok = 20.0   # m
                default_L_wag = 20.0   # m
            ...
            mass_points_lok = st.radio(
                "Mass points per locomotive",
                [2, 3],
                index=1,
                horizontal=True
            )
            mass_points_wagon = st.radio(
                "Mass points per wagon",
                [2, 3],
                index=0,
                horizontal=True
            )
...
with col1:
    ...
    st.metric("Mass points", n_masses)
...
with col2:
    if run_btn:
        ...
        # Metrics (building-focused)
        c1, c2, c3 = st.columns(3)
        c1.metric("Max Force (building)", f"{F_total.max()/1e6:.2f} MN")
        c2.metric("Max Penetration (building)", f"{u_pen.max():.2f} mm")
        c3.metric("Max Acceleration (front mass)", f"{np.nanmax(a_front):.1f} g")
        
        # Plots
        fig = make_subplots(
            rows=4, cols=1,
            subplot_titles=('Force (building reaction)',
        ...
        fig.add_trace(
            go.Scatter(
                x=export_df['Time_ms'], y=export_df['Impact_Force_MN'],
        ...
        fig.add_trace(
            go.Scatter(
                x=export_df['Time_ms'], y=export_df['Penetration_mm'],
        ...
        fig.add_trace(
            go.Scatter(
                x=export_df['Time_ms'], y=export_df['Acceleration_g'],
        ...
        # Building hysteresis (F vs penetration)
        fig.add_trace(
            go.Scatter(
        ...
        c2.download_button(
            "📝 TXT",
            export_df.to_string(index=False).encode(),
            "results.txt",
            "text/plain",
            use_container_width=True
        )
